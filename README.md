...
1. **Freedom's Dawn** — a sunrise hero with a rising/waving CSS-only flag, plus a `:checked`-driven sparkler toggle.
2. **Patriot's Plaza** — a Main Street parade with `translateX` marchers and a `:checked` radio group that swaps between three float descriptions.
3. **United We Stand** — a night-sky fireworks finale with a `:checked`-driven launcher (rocket + expanding burst) and a closing statement.
